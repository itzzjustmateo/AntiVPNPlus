...
public enum ProtocolVersion {
    ...
    MINECRAFT_1_21(766, "1.21");

    /* ---------------------------------------------------------------------- */
    /* Static lookup                                                          */
    /* ---------------------------------------------------------------------- */

    private static final Map<String, ProtocolVersion> BY_VERSION;

    static {
        Map<String, ProtocolVersion> map = new HashMap<>();
        for (ProtocolVersion version : values()) {
            for (String name : version.names) {
                map.put(name, version);
            }
        }
        BY_VERSION = Collections.unmodifiableMap(map);
    }
    ...
    public static ProtocolVersion fromMinecraftVersion(String version) {
        if (version == null || version.isEmpty()) {
            return UNKNOWN;
        }
        return BY_VERSION.getOrDefault(version, UNKNOWN);
    }
}
